...
**|                                                                    |**
**+--------------------------------------------------------------------+**
* FILE:   	ST_ATA_Multi_Process.c
*
* Brief:  	ATA MultiTasking Support.
*
* Platform: 	Linux 2.6
*
* Author: 	Anand Patil
...
Uint32 size = WRITEREADDATA;				/* Data size */
static Uint32 mode,processnum;
static int Thread_usage=0;
static int RetVal=ST_FAIL;

void ST_ATA_MultiThread_parser(void);
void ST_ATA_multiProcess_parser(void);
...
static void ST_Multi_ProcThread_parser(void)
{
	printTerminalf("MultiProcess: ATA:: Enter the Write (0), Read (1) or Write and Read (2) mode, Quit (3)\n");
	...
	printTerminalf("MultiProcess/Thread: ATA:: Enter number of bytes to Write / Read/ Write & Read\n");
    	scanTerminalf("%d", &size);

	printTerminalf("MultiProcess/Thread: ATA:: Enter number of  Processes/Threads to Spawn\n");
...
/* Create child process for Write */
			...
			switch((pid[loop]=fork()))
			{
				case 0: // Child Process
					{
						switch(mode)
						{
							...
							{
								writetest();
						   		printTerminalf("\nATA multiProcess_parser: Write Process %d is spawned\n",getpid());							     
								exit(0);
								break;
							}	
							case 1 :
							{
								readtest();
								printTerminalf("\nATA multiProcess_parser: Read Process %d is spawned\n",getpid());
							...
							{
								writereadtest();
								printTerminalf("\nATA multiProcess_parser: Read/Write Process %d  is spawned\n",getpid());						     
								exit(0);
								break;
							}
				...
				case -1: // Child process creation Failed
						{
							printTerminalf("ATA multiProcess_parser: Error creating Process\r\n");
				...
				default: // Parent process
						{
							...
							//wait(0);
							printTerminalf("ATA multiProcess_parser: Parent process Executed\r\n");
	...
	if(Failiure_Flg)
		printTerminalf("ST_ATA_multiProcess_parser:Completed with Failiure\n");
	else
		printTerminalf("ST_ATA_multiProcess_parser:Completed Successfully\n");		
...
//This snippet is for waiting for all the Spawned Threads 
	...
	Thread_usage=0;
	if(Failiure_Flg)
		printTerminalf("ST_ATA_MultiThread_parser:Completed with Failiure\n");
	else
		printTerminalf("ST_ATA_MultiThread_parser:Completed Successfully\n");		
