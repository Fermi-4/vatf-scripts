...
static int opmode1=ST_POLLED;
static int trig1=0; //If it is polled mode Trigger is not configured 
static int dir1=0;
static int irq_num1=;
static int gio_rx=GIO_V18_29; //Mux information -> CLKX /GPIO -> DEMUX -> [McBSP (0) ] available on DC3 pin 12
static int opmode2=ST_POLLED;
...
void ST_GPIO_unregIRQ(struct ST_GPIOAttrs *gio)
{
	...
	if(gio->direction)
	{
		status = free_irq(gio->irq_num, NULL);
		if (status < 0)
			printk("<1>  ST_GPIO_regIRQ : Failed to Free IRQ %d  \n",gio->irq_num);
	}
...
void ST_GPIO_Config(struct ST_GPIOAttrs *gio)
{
	...
	{
		if(trig1==ST_RISING_EDGE)
			gio_set_rising_edge(gio->trig_edge,1);
		else
			gio_set_falling_edge(gio->trig_edge,1);
	}
...
void ST_GPIO_GetDirRegInfo(int gio)
{
	...
	{
		default:
		case (0x10+DAVINCI_GPIO_BASE) : 
				printk("<1> GPIO direction 0 and 1 Banks:\t %x\n", *reg);
				break;
				
		case (0x38+ DAVINCI_GPIO_BASE):
				printk("<1> GPIO direction 2 and 3 Banks:\t %x\n", *reg);
				break;
				
		case (0x60+DAVINCI_GPIO_BASE) :
				printk("<1> GPIO direction 4 and 5 Banks:\t %x\n", *reg);
				break;
	}
...
void ST_GPIO_GetOutDataRegInfo(int gio)
{
	...
	{
		default:
		case (0x14+DAVINCI_GPIO_BASE) : 
				printk("<1> GPIO OutData 0 and 1 Banks:\t 0x%x\n", *reg);
				break;
				
		case (0x3C+ DAVINCI_GPIO_BASE):
				printk("<1> GPIO OutData 2 and 3 Banks:\t 0x%x\n", *reg);
				break;
				
		case (0x64+DAVINCI_GPIO_BASE) :
