...
/****************************************************************************
 * Function            		- kSt_edma3_memtomemcpytest_dma
			      ...
			      int event_queue)
{
	...
	for (trial = 0; trial <= MAX_TRIALS; trial++)
	{
		...
		if (sync_mode == ASYNC) {
			...
			DBG_PRINT_ERR( (" Invalid Transfer mode "));
		}
		if (FAILURE == result) {
			DBG_PRINT_ERR(("test_dma::davinci_request_dma failed for dma_ch, error:%d",result));
			return result;
		}
		...
		for (i = 0; i < numenabled; i++) {
			...
			if (result != 0) {
				DBG_PRINT_ERR(("test_dma: kSt_davinci_start_dma failed "));
				break;
			}
			...
			if (irqraised1 < 0) {
				...
				DBG_PRINT_ERR(("test_dma: Event Miss Occured!!!"));
				break;
			}
		}
	...
	if (Istestpassed == 1u) {
		DBG_PRINT_TRC0( ("test_dma: Transfer controller/event_queue: %d", event_queue));
		DBG_PRINT_TRC0( ("test_dma: Mode: %d  0 -> ASYNC, 1 -> ABSYNC", sync_mode));		
		if (performance == 1) { 

			DBG_PRINT_TRC0( ("test_dma: Time Elapsed in usec: %d", (total_Time/MAX_TRIALS)));
		} else {
			DBG_PRINT_TRC0(("test_dma: EDMA Data Transfer Successfull "));
		}
	} else {
		DBG_PRINT_TRC0(("test_dma: EDMA Data Transfer Failed "));
	}
