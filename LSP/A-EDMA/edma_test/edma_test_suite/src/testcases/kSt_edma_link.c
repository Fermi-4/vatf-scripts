...
/****************************************************************************
 * Function            		- kSt_edma3_memtomemcpytest_dma_link
				   ...
				   int event_queue)
{
	...
	for (trial = 0; trial <= MAX_TRIALS; trial++)
	{
		...
		desbidx = acnt;
        	...
        	if (FAILURE == result) {
                	DBG_PRINT_ERR(("test_dma::davinci_request_dma failed for dma_ch1, error:%d",result));
                	return result;
        	}
		...
		if (FAILURE == result) {
			DBG_PRINT_ERR(("test_dma_link::davinci_request_dma failed for dma_ch2, error:%d\n", result));
			return result;
		}
		...
		for (i = 0; i < numenabled; i++) {
			...
			if (result != 0) {
				DBG_PRINT_ERR(("test_dma_link: kSt_davinci_start_dma failed "));
				break;
			}
			...
			if (irqraised1 < 0) {
				...
				DBG_PRINT_ERR(("test_dma_link:  Event Miss Occured!!!"));
				break;
			}
		}
		...
		if (0 == result) {
			for (i = 0; i < (acnt * bcnt * ccnt); i++) {
				if (dmabufsrc1[i] != dmabufdest1[i]) {
					DBG_PRINT_ERR(("test_dma_link(1): Data write-read matching failed at = %u", i));
					result = -1;
					Istestpassed1 = 0u;
					break;
				}
			...
			for (i = 0; i < (acnt * bcnt * ccnt); i++) {
				if (dmabufsrc2[i] != dmabufdest2[i]) {
					DBG_PRINT_ERR(("test_dma_link(2): Data write-read matching failed at = %u", i));
					result = -1;
					Istestpassed2 = 0u;
					break;
				}
	...
	if ((Istestpassed1 == 1u) && (Istestpassed2 == 1u)) {
		DBG_PRINT_TRC0( ("test_dma_link: Transfer controller/event_queue: %d", event_queue));
		DBG_PRINT_TRC0(("test_dma_link: Mode: %d  0 -> ASYNC, 1 -> ABSYNC", sync_mode));	
		DBG_PRINT_TRC0(
		    ("test_dma_link: EDMA Data Transfer Successfull"));
	} else {
		DBG_PRINT_TRC0(
		    ("test_dma_link: EDMA Data Transfer Failed"));
	}
