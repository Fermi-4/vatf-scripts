...
/****************************************************************************
 * Function            		- kSt_edma3_memtomemcpytest_dma_chain
				    ...
				    int event_queue)
{
	...
	for (trial = 0; trial <= MAX_TRIALS; trial++)
	{
		...
		if (FAILURE == result) {
			DBG_PRINT_ERR(("test_dma_chain::davinci_request_dma failed for dma_ch1, error:%d", result));
			return result;
		}
		...
		if (FAILURE == result) {
			DBG_PRINT_ERR(("test_dma_chain::davinci_request_dma failed for dma_ch2, error:%d", result));
		...
		for (i = 0; i < numenabled; i++) {
			...
			if (result != 0) {
				DBG_PRINT_ERR(("test_dma_chain: kSt_davinci_start_dma failed "));
			...
			*/
			...
			if (irqraised2 < 0) {
				...
				DBG_PRINT_ERR(("edma3_test_with_chaining: "
					   "Event Miss Occured!!!"));
				break;
			}
		...
		for (i = 0; i < (acnt * bcnt * ccnt); i++) {
			if (dmabufsrc1[i] != dmabufdest1[i]) {
				DBG_PRINT_ERR(("test_dma_chain(1): "
		...
		for (i = 0; i < (acnt * bcnt * ccnt); i++) {
			if (dmabufsrc2[i] != dmabufdest2[i]) {
				DBG_PRINT_ERR(("test_dma_chain(2): "
					"Data write-read matching failed at = %u",i));
				Istestpassed2 = 0u;
				result = -1;
				break;
	...
	if ((Istestpassed1 == 1u) && (Istestpassed2 == 1u)) {
		DBG_PRINT_TRC0( ("test_dma_chain: "
			"Transfer controller/event_queue: %d", event_queue));
		DBG_PRINT_TRC0( ("test_dma_chain: "
			"Mode: %d  0 -> ASYNC, 1 -> ABSYNC", sync_mode));	
		DBG_PRINT_TRC0(("test_dma_chain: "
	   	 	"EDMA Data Transfer Successfull"));
	} else {
		DBG_PRINT_TRC0(
		    ("test_dma_chain: EDMA Data Transfer Failed"));
	}
	return result;
}
