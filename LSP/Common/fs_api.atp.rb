class Fs_apiTestPlan < TestPlan
  ...
  def get_outputs(params)
    {
      ...
      'description'    => "Verify Filesystem API -- #{params['fs_api']} for #{params['fs_type'].upcase}",

      'testcaseID'      => "fs_func_api_" + "%04d" % "#{@current_id}",
  ...
  def get_cmd(fs_api, mnt_point, device_node, fs_type)
    ...
    rtn = case fs_api
     ...
     # when 'mtd_info': "./st_parser nand nand_ioctl #{device_node} mtd_info `++Success::\\s+Get MTD Information Success--Failed::\\s+Get MTD Information Failed`" 
     #when 'set_oob': "./st_parser nand nand_ioctl #{device_node} set_oob `++Success::\\s+Set OOB Success--Failed::\\s+Set OOB Failed`"  
     #when 'get_badblk': "./st_parser nand nand_ioctl #{device_node} get_badblk `++Success::\\s+Get Bad block information Success--Failed::\\s+Get Bad Block Information Failed`"  

      when 'mount': "cd #{mnt_point};mkdir -p #{mnt_point}/mnt/sub;[dut_timeout\\=120];flash_eraseall #{device_node.delete('block')};mount -t #{fs_type} #{device_node} #{mnt_point}/mnt/sub;mount`++#{mnt_point}/mnt/sub`"
