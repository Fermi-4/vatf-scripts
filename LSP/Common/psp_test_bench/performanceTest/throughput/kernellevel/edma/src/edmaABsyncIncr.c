...
/**
 *  \file    st_edma_ABSync_INCR.c
 *
 *  \brief   This file Tests AB-Sync INCR mode transfer for DMA Channels
 *  
 ...
 * ==========================================================
 */
static void callback(s16 lch, u16 ch_status, void *data)
{
 ...
 * ==========================================================
 */
s32  st_edma_ABsync_INCR_init(void)
{
    s16 index = 0;
#ifndef AUTOMATION
    DBG_PRINT_TRC(("\n******************************************************************"));
    DBG_PRINT_TRC(("\nEDMA:NOTIFY	:DMA Memory to Memory Transfer AB-Sync INCR\n"));
    DBG_PRINT_TRC(("\n******************************************************************\n"));
#endif
    ...
    {
        if ((edmaTransferparams[index].ACnt >= 0)&&(edmaTransferparams[index].BCnt >= 0)&&(edmaTransferparams[index].CCnt >= 0))
        {
#ifndef AUTOMATION
            DBG_PRINT_TRC(("\nACnt =%d :BCnt =%d :CCnt =%d",edmaTransferparams[index].ACnt,edmaTransferparams[index].BCnt,edmaTransferparams[index].CCnt));
#else      
        DBG_PRINT_TRC(("edma: absync: A Count: %d\n", edmaTransferparams[index].ACnt));
        DBG_PRINT_TRC(("edma: absync: B Count: %d\n", edmaTransferparams[index].BCnt));
        DBG_PRINT_TRC(("edma: absync: C Count: %d\n", edmaTransferparams[index].CCnt));
#endif
            st_edma_ABsync_incr(edmaTransferparams[index].ACnt , edmaTransferparams[index].BCnt , edmaTransferparams[index].CCnt);
        }
        else
        {
            DBG_PRINT_TRC(("\nEDMA:NOTIFY	:Finished DMA Memory to Memory Transfer AB-Sync INCR\n"));
            break;
        }
    }
    return 0;
 ...
 * ================================================================
 */
void st_edma_ABsync_incr(u32 ACnt,u32 BCnt,u32 CCnt)
{
    s32 dma_ch = 0;
    ...
    result = davinci_request_dma(EDMA_DMA_CHANNEL_ANY, "AB-Sync_DMA0",(void*)callback, NULL, &dma_ch, &tcc, EVENTQ_0 );
    ...
    if(Failure_flag)
        DBG_PRINT_TRC(("\nEDMA:FAIL	:DMA, AB-Sync,INCR, Data Transfer Failed\n"));		
    else
#ifndef AUTOMATION
        DBG_PRINT_TRC0(("\nEDMA:PASS	:DMA, AB-Sync,INCR, Data Transfer Successfull SIZE=%dBytes\n",size));
#else
                ;
#endif
    ...
    {
#ifndef AUTOMATION
        DBG_PRINT_TRC(("\nEDMA: Avg Time Elapsed is %d micro-sec for %d Bytes\n", (total_Time/MAX_TRIALS),size));
#else      
        DBG_PRINT_TRC(("edma: absync: Application buffer Size in Kbits: %d\n", size));
        DBG_PRINT_TRC(("edma: absync: Time Elapsed in usec: %d\n", (total_Time/MAX_TRIALS)));
 ...
 * ==========================================================
 */
void st_edma_ABsync_INCR_exit(void)
{
	st_memory_free(ONE_BUFFER);
#ifndef AUTOMATION
	DBG_PRINT_TRC(("EDMA:NOTIFY	:Exiting DMA Memory to Memory Transfer AB-Sync INCR\n"));
#endif
}

module_init(st_edma_ABsync_INCR_init);
module_exit(st_edma_ABsync_INCR_exit);
