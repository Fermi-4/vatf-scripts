 ...
 *******************************************************************************/
/**
 *  \file   audioReadThroughput.c
 *
 *  \brief  This file implements the audio read throughput test case in sync
 *  mode
 *
...
int throughputAudioRead(int numArgs, const char ** argv)
{
    int audioFD              = 0;
    int readRet              = 0;
    int status               = 0;
    ...
    do
    {
        ...
        audioFD = open((const char *)devNode, O_RDONLY | O_SYNC);
        if(-1 == audioFD)
        {
            perror("\nopen ");
            break;
	...
	/* Set the number of channels */
       ...
       /* Start CPU Load calcaulation */ 
        ...
        for(loopIndex = 0 ; loopIndex < loopCount; loopIndex++)
        {
            ...
            readRet = read(audioFD, buffPtr, bsize);
            if(bsize != readRet)
            {
                perror("\nread");
            }
...
#else      
        printf("audio: read: Word Length in bits: %d\n", numBits);
#endif
        }
...
#else      
        printf("audio: read: No. of channels per sample: %d\n", numChans);
#endif
        }
        ...
        if (-1 == status)
        {
            ...
            PERFLOG("SOUND_PCM_READ_RATE: %d\n",
                    retSamplingRate);
#else      
        printf("audio: read: Sampling Rate in Hz: %d\n", retSamplingRate);
#endif
        }
...
#else      
        printf("audio: read: Duration in Sec: %lf\n", elapsedSecs);
        printf("audio: read: No. of bits/Sec: %.0lf\n", ((double)(totalSize * 8) / elapsedSecs));
#endif
        if((percentageCpuLoad >= 0) && (percentageCpuLoad <= 100))
            printf("audio: read: percentage cpu load: %.2f%%\n", percentageCpuLoad);
