 ...
 *******************************************************************************/
/**
 *  \file   audioReadToFileThroughput.c
 *
 *  \brief  This file implements the audio read throughput test case in sync
 *  mode. The data recorded is written to a file
 *
...
int throughputAudioReadToFile(int numArgs, const char ** argv)
{
    int audioFD              = 0;
    int recordFD             = 0;
    int writeRet             = 0;
	...
	/* Set the number of channels */
       ...
       /* Start CPU Load calcaulation */ 
        ...
        for(loopIndex = 0 ; loopIndex < loopCount; loopIndex++)
        {
            ...
            /* Write data to file */
            writeRet = write(recordFD, buffPtr, bsize);
            if(bsize != writeRet)
            {
                perror("\nfwrite");
            }
...
#endif
        /* close the file descriptor so that the data is flushed*/
        status = close(recordFD);
        if(-1 == status)
        {
            perror("\nfclose ");
        }
