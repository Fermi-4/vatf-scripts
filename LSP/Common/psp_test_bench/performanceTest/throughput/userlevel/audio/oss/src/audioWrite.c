...
int throughputAudioWrite(int numArgs, const char ** argv)
{
    int audioFD              = 0;
    int writeRet             = 0;
    int status               = 0;
    ...
    do
    {
        ...
        buffPtr = (short int * ) perfAllocateBuffer(bsize * (sizeof(char)));
        if (NULL == buffPtr)
        {
            break;
        }
        ...
        audioFD = open((const char *)devNode, O_WRONLY | O_SYNC);
        if(-1 == audioFD)
        {
            perror("\nopen ");
            break;
	...
	/* Set the number of channels */
        ...
        for (loopIndex = 0; loopIndex < bsize/4; loopIndex++)
        {
            ...
            buffPtr[(2*loopIndex)] = scale (y); /* left channel */
            buffPtr[(2*loopIndex)+1] = scale (x); /* right channel */
        }
