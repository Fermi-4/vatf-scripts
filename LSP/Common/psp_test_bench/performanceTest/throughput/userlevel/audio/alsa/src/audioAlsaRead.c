...
int throughputAudioAlsaRead(int numArgs, const char ** argv)
{
    int status               = 0;
    int loopIndex            = 0;
    int loopCount               = 0;
...
//        printf("audio: read: Theoretical throughput = %d bits/sec\n", numBits * numChans * retSamplingRate);
#endif
...
static void read_audio_usage(void)
  {   
    PERFLOG("./pspTest ThruPut FRaudioalsaread [Capture device] [Sampling Rate] [Buffer Size] [Total Data Size]\n");
  }
