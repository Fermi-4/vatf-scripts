 ...
 *******************************************************************************/
/******************************************************************************
FILE            : audioAlsaWriteFromFile .c
BRIEF           : Function to calculate the Through put of Audio Driver during Playback.
                  The data to be written to the device is read from a file.
PLATFORM        : Linux
...
int throughputAudioAlsaWriteFromFile(int numArgs, const char ** argv)
{
    int status                  = 0;
    int loopIndex               = 0;
    int loopCount               = 0;
    ...
    unsigned long int * write_buffPtr = 0;
    char device[15] = {'\0',};
    int playbackFD           = 0;
    int readRet              = 0;
   ...
   do
    {
       loopCount = totalSize/bsize;
        ...
        if((write_buffPtr = (unsigned long int * ) perfAllocateBuffer(bsize)) == NULL)
            {
        ...
        for(loopIndex = 0 ; loopIndex < loopCount; loopIndex++)
        {
            ...
            /* Read data from file */
            readRet = read(playbackFD, write_buffPtr, bsize);
            if(bsize != readRet)
            {
                perror("\nfread");
            }
...
//        printf("audio_file: write: Theoretical throughput = %d bits/sec\n", numBits * numChans * retSamplingRate);
#endif

        if((percentageCpuLoad >= 0) && (percentageCpuLoad <= 100))
            printf("audio_file: write: percentage cpu load: %.2f%%\n", percentageCpuLoad);
...
static void write_audio_usage(void)
  {   
    PERFLOG("./pspTest ThruPut FRaudioalsawrite [Audio File Name] [Playback device] [Sampling Rate] [Buffer Size] [Total Data Size]\n");
  }
