 ...
 *******************************************************************************/
/******************************************************************************
FILE            : audioAlsaWrite .c
BRIEF           : Function to calculate the Through put of Audio Driver during Playback.
PLATFORM        : Linux
...
static void write_audio_usage(void);
static short int scale (float data);


/************************** Main Function Definition ***************************/

/****************************************************************************
 * Function             - throughputAudioAlsaWrite
...
int throughputAudioAlsaWrite(int numArgs, const char ** argv)
{
    int status               = 0;
    int loopIndex            = 0;
    int loopCount               = 0;
    ...
    if((strcmp(device,"plughw:0,0") != 0) && (strcmp(device,"plughw:1,0") != 0))
        {
            PERFLOG("%s is not a valid Audio Playback Device.\n", device);
            write_audio_usage();
            return(-1);
        }
    ...
    do
    {
        ...
        if ((status = snd_pcm_open (&pcm_handle, device, SND_PCM_STREAM_PLAYBACK, 0)) < 0) 
        {
        ...
        if((write_buffPtr = (short int * ) perfAllocateBuffer(bsize)) == NULL)
        {
            PERFLOG("Memory allocation for Buffer Failed.\n");
            return(-1);
        }

        for (loopIndex = 0; loopIndex < bsize/4; loopIndex++)
        {
            float y = sin( 2 * M_PI * LEFT_SINE_FREQ * loopIndex / samplingRate );
            float x = sin( 2 * M_PI * RIGHT_SINE_FREQ * loopIndex / samplingRate );

            write_buffPtr[(2*loopIndex)] = scale (y); /* left channel */
            write_buffPtr[(2*loopIndex)+1] = scale (x); /* right channel */
       ...
       /* Start CPU Load calcaulation */ 
        ...
        for(loopIndex = 0 ; loopIndex < loopCount; loopIndex++)
        {
           ...
           if((status = (snd_pcm_writei(pcm_handle, write_buffPtr, bsize/4))) < 0)
                {
                if (status == -EAGAIN)
                        {
                ...
                {
                    PERFLOG("Write from Device Failed\n");
                            perfFreeBuffer(write_buffPtr);
                            return(-1);
                        }
                }            
            else if(status != bsize/4)
            {
                PERFLOG("failed to write all the samples %ld(%d)\n", status, bsize/4);
            }
        
            if((strcmp((snd_pcm_state_name(snd_pcm_state(pcm_handle))), "RUNNING")) != 0 )
              {
                PERFLOG("Write Device State: %s\n", snd_pcm_state_name(snd_pcm_state(pcm_handle)));
                perfFreeBuffer(write_buffPtr);
                return(-1);
              }
        ...
        else 
        {
            ...
            PERFLOG("NUMBER OF BITS : %d\n", numBits);
#else      
            printf("audio: write: Word Length in bits: %d\n", numBits);
#endif
        }
        ...
        else 
        {
            ...
            PERFLOG("CHANNELS : %d\n", numChans);
#else      
            printf("audio: write: No. of channels per sample: %d\n", numChans);
#endif
        }
...
#else      
        printf("audio: write: Sampling Rate in Hz: %d\n", retSamplingRate);
#endif
        }
...
#else      
        printf("audio: write: Duration in Sec: %lf\n", elapsedSecs);
        printf("audio: write: No. of bits/Sec: %.0lf\n", ((double)(totalSize * 8) / elapsedSecs));
//        printf("audio: write : Theoretical throughput = %d bits/sec\n", numBits * numChans * retSamplingRate);
#endif
...
static void write_audio_usage(void)
  {   
    PERFLOG("./pspTest ThruPut FRaudioalsawrite [Playback device] [Sampling Rate] [Buffer Size] [Total Data Size]\n");
  }
