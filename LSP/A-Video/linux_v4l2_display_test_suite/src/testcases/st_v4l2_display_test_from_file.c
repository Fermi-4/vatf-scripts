...
/*	File Name :   st_v4l2_resolution_tests.c
 ...
 * Note                 - None
 ****************************************************************************/
void st_v4l2_display_from_file_test(struct v4l2_display_testparams *testoptions,  char* test_id,char* filename)
{
    ...
    {  
        ...
        if(fd <= 0)
        {
            DBG_PRINT_ERR(("Couldn't open file %s",filename));
            status = FAILURE;
            break;
        } 

        DBG_PRINT_TRC0(("Opened file %s",filename)); 
        ...
        /* Query buffer */
        retVal = st_v4l2_display_query_buffer_mmap_interface_file(st_dev);
        ...
        /* Request buffer */
        retVal = st_v4l2_display_read_buffer_from_file_interface(fd);
        if (SUCCESS != retVal)
        {
            DBG_PRINT_ERR(("Read from file Failed |"));
            status = FAILURE;
            break;
        }

        DBG_PRINT_TRC0(("Read from file Passed |"));

        /* Enqueue buffers */
        retVal = st_v4l2_display_qbuf_interface(st_dev);
        if (SUCCESS != retVal)
        {
        ...
        /* Display frames */       
        retVal = st_v4l2_display_from_file(st_dev,fd,noofframes);
        if (SUCCESS != retVal)
        {
