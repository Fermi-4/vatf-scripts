...
#include <time.h>
//#include <media/davinci/dm355_vpfe.h>	/*kernel header file, prefix path comes from makefile */
//#include <asm/arch/dm350_aew.h>
#include <asm/arch/dm355_aew.h>
...
int aew_Functional_TC2_1(void)
{
	...
	if(ioctl(fd, AEW_S_PARAM, config) == -1)
	{
...
int aew_Functional_TC2_2(void)
{
	...
	printf("\n Window Vertical Count : %d",
	       config1->window_config.vt_cnt);
	...
	if( (config1->window_config.vt_start == config->window_config.vt_start) && (config1->window_config.hz_line_incr == config->window_config.hz_line_incr))
	{
...
int aew_Functional_TC2_3(void)
{
	...
	if(ioctl(fd, AEW_S_PARAM, config) == -1)
	{
		printf("Failed AEW_ENABLE \n");
	}
	else
	{
		printf("Success AEW_ENABLE \n");
	}
...
int aew_Functional_TC2_4(void)
{
        ...
        if(ioctl(fd, AEW_DISABLE) == -1)
	{
		printf("Failed AEW_DISABLE \n");
	}
	else
	{
		printf("Success AEW_DISABLE \n");
	}
...
int aew_Functional_TC5_1(void)
{
	...
	ioctl(fd, AEW_ENABLE);
	size_buff =
	    (config->window_config.hz_cnt ) * (config->window_config.vt_cnt) * 16;
...
int aew_Functional_TC5_2(void)
{
	...
	printf("\nbefore thread ");
	pthread_create(&ch2_thread, NULL, (void *) aew_TC5_2_Test,
		       (void *) 1);
	pthread_create(&ch1_thread, NULL, (void *) aew_TC5_2_Test,
		       (void *) 2);
...
void * aew_TC5_2_Test(int id)
{
	...
	printf("\n SIZE OF BUFFER IS %d",size_buff_thread);
		...
		result =
		    read(fd_thread, (short int *) buffer_stat1,
			 size_buff_thread);
		...
		printf("\n HI i m printing for thread %d",id);
		    if(id == 1)
            {		            
		        aew_DumptoFile(buffer_stat1,size_buff_thread ,"tc5_2_1stthread.txt");
            }
            else
            {                
             aew_DumptoFile(buffer_stat1,size_buff_thread ,"tc5_2_2ndthread.txt");   		         
            }
        }
	#endif			
	if (buffer_stat1)
