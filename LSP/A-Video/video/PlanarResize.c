...
/*      int j;*/
	...
	/*Channel 3*/
	rsz_params_t ch3_params, ch3_params1;
	rsz_buffer_t ch3_bufd;
	rsz_reqbufs_t ch3_req_outbufs, ch3_req_inbufs;
	rsz_resize_t ch3_resize;
/*      int j;*/
	...
	{
	if (ioctl(ch1_fd, RSZ_REQBUF, NULL) == -1) {
	...
	{
	if (ioctl(ch1_fd, RSZ_QUERYBUF, NULL) == -1) {
		printf("\nPassed Negative Tests \n");
		close(ch1_fd);
		exit(-1);
	}
	...
	{
	if (ioctl(ch1_fd, RSZ_S_PARAM, NULL) == -1) {
		printf(" Passed Negative Tests \n");
		close(ch1_fd);
		exit(-1);
	}
	}
	...
	/* else use  gRDRV_reszFilter7TapHighQuality*/
	for (i = 0; i < 32; i++)
		ch3_params.hfilt_coeffs[i] =
		    gRDRV_reszFilter4TapHighQuality[i];

	for (i = 0; i < 32; i++)
		ch3_params.vfilt_coeffs[i] =
		    gRDRV_reszFilter4TapHighQuality[i];
...
/*Channel 3 resizing*/
	...
	fwrite((void *) ch1_outBuf, 1, (OUT_HEIGHT * OUT_WIDTH),
	       outputimage);
