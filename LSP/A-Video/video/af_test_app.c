...
//#include <asm-arm/arch/dm350_af.h>
#include <asm/arch/dm355_af.h>
...
void *buffer_stat;
int startLoopCnt = 10;
//static int Sensor_TurnOnOff(int on_off);
/*
static int Init_sensor(void)
{
        ...
        int on_off = 1; // On
        printf("\nfdCapture = 0x%x\n",fdCapture);
        if (-1 == ioctl(fdCapture, VIDIOC_SENSOR_RESET, &arg)) {
                printf("VIDIOC_SENSOR_RESET FAIED ????\n");
        }
        printf("\nfdCapture = 0x%x\n",fdCapture);
        printf("RESET Done in App\n");
        if (-1 == ioctl(fdCapture, VIDIOC_SENSOR_SETMODE, &mode)) {
                printf("VIDIOC_SENSOR_SETMODE FAIED ????\n");
        }
        printf("\nfdCapture = 0x%x\n",fdCapture);
        printf("SETMODE Done in App\n");
        if (-1 == ioctl(fdCapture, VIDIOC_SENSOR_TURNONOFF, &on_off)) {
                printf("VIDIOC_SENSOR_TURNONOFF FAIED ????\n");
        }
        printf("\n fdCapture = %d\n",fdCapture);
        return 0;
...
#endif
/*static int set_sensor_gain(int sensor_gain)
{
	printf("Sensor Gain =%d\n",sensor_gain);	
	if (-1 == ioctl(fdCapture, VIDIOC_SENSOR_GAIN, &sensor_gain)) {
                printf("VIDIOC_SENSOR_GAIN FAILED ????\n");
        }
	return 0;
}
static int Sensor_TurnOnOff(int on_off)
...
int af_Functional_TC2_1(void)
{
	...
	if(ioctl(fd, AF_S_PARAM, config) == -1)
	{
		printf("Failed S_PARAM \n");	
		return -1;
	}
	//printf("No of bytes that are required by the application  %d",
	//       result);
	/* Disable the engine */
	ioctl(fd, AF_DISABLE);
	if (config)
		free(config);
	printf("Success S_PARAM \n");
	return 0;
...
int af_Functional_TC2_2(void)
{
	...
	//       result);
	if(ioctl(fd, AF_G_PARAM, config1) == -1)
	{
		printf("Failed G_PARAM \n");
		return -1;
	}
	printf
	    ("\n ************************Paxel Paramters***********");
	...
	printf("\nPaxel Horizontal Start : %d",
	       config1->paxel_config.hz_start);
	printf("\nPaxel Vertical Start : %d",
	       config1->paxel_config.vt_start);
	printf("\nPaxel Line Increment : %d",
	       config1->paxel_config.line_incr);
	printf
	    ("\n *****************COEFFICIENT SET0*********");
	for (index = 0; index < 11; index++) {
	...
	printf("\n%d\t\t%d\t\t%d", config->mode, config->alaw_enable,config->rgb_pos);
	//printf("\nNo of bytes that are required by the applicatiob  %d",
	//       result);
	if( (config1->paxel_config.vt_start == config->paxel_config.vt_start) && (config1->paxel_config.line_incr == config->paxel_config.line_incr))
	{
		printf("Success G_PARAM \n");
	}
	else
	{
		printf("Failed G_PARAM \n");
...
int af_Functional_TC2_3(void)
{
	...
	if(ioctl(fd, AF_ENABLE) == -1)
	{	
		printf("Failed AF_ENABLE \n");
		return -1;
	}
...
{	int fd;
	...
	if(ioctl(fd, AF_DISABLE) == -1)
	{
		printf("Failed AF_DISABLE \n");
		return -1;
	}
	/* Close Driver */
	close(fd);
	printf("Success AF_DISABLE \n");
    return 0;
}
...
=====================af_Functional_TC2_5===========================
*/
/*Function to test open and close 
*/
...
=====================af_Functional_TC2_6===========================
*/
/*Function to test open and close multiple times 
*/
...
static int SetDataFormat(void)
{
	...
	cur_std = prev_std = V4L2_STD_NTSC;// V4L2_STD_PANAMN39850_QVGA;
...
int af_Functional_TC5_1(void)
{
    ...
    buff_size  = config.paxel_config.hz_cnt * config.paxel_config.vt_cnt *32;
...
int af_Functional_TC5_2(void)
{
	...
	printf("\nbefore thread ");
	pthread_create(&ch2_thread, NULL, (void *) af_TC5_2_Test,
		       (void *) 1);
	pthread_create(&ch1_thread, NULL, (void *) af_TC5_2_Test,
		       (void *) 2);
...
void* af_TC5_2_Test(int id)
{
	...
	printf("\n SIZE OF BUFFER IS %d",size_buff_thread);
		...
		if (size_buff_thread > 0)
			buffer_stat1 = 
			    malloc(size_buff_thread);
		result =
		    read(fd_thread, (void *) buffer_stat1,
			 size_buff_thread);
		...
		printf("\n HI i m printing for thread %d",id);
		    if(id == 1)
            {		            
		        af_DumptoFile(buffer_stat1,size_buff_thread ,"tc5_2_1stthread.txt");
            }
            else
            {                
             af_DumptoFile(buffer_stat1,size_buff_thread ,"tc5_2_2ndthread.txt");   		         
            }
        }
...
int main(int argc, char *argv[])
{
	    ...
	    
    {
	  ...
	  Init_sensor();
			if ( sensor_gain != 0 )
			{
				set_sensor_gain(sensor_gain);
			}
			else
			{
				set_sensor_gain(1500);	
