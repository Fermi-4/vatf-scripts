/*
 * Filename: setblinkbit.c
 * 
 * This sample application writes to the blink bit of the OSD1 framebuffer.
 * OSD1 is in attribute mode. Setting the blink bit enables OSD0 blinking.
*/
#include <stdio.h>
#include <sys/ioctl.h>
#include <fcntl.h>
...
int main(int argc,char *argv[])
{
 ...
 //blend = (((uint8_t) 4 & 0xf) << 4) | ((uint8_t) 4);
 // You do not have to set the all the 16 bits. Just the blink bit, if you know which one 
 // that is. Setting it to 0xff will make all the OSD0 pixels to blink
 blend = 0xff;
