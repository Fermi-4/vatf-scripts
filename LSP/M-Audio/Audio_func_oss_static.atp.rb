...
class Audio_func_oss_staticTestPlan < TestPlan
  ...
  def get_manual()
    ...
    tc = [
      {
        'description'  =>  "Verify that the OSS audio loopback works fine with audio driver module loaded dynamically.",
        'testcaseID'   => 'Oss_dynamic_mod_0001',
  ...
  def get_outputs(params)
    {
        'testcaseID'     => "Audio_func_oss.#{@current_id}",
        'description'	=> "Audio OSS Functionality test with #{get_input(params['input'])} input and #{get_output(params['output'])} Output with a sampling rate of #{params['srate']} Hertz and frame size of #{params['frame_sz']} Bytes.",
        'iter'         => 1,
        'bft'          => true,
        'basic'        => true,
        'ext'          => false,
