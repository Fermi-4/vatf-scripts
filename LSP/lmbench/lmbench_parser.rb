...
def get_metrics
  perf_metrics = [
   ...
   {'name' => 'TCP Latency Using LocalHost',
    'regex' => 'TCP latency using localhost\s*:\s*(\d*.\d*)\s*microseconds',
    'units' => 'microsec',
   },
   {'name' => 'AF_UNIX Sock Stream Latency',
    'regex' => 'AF_UNIX sock stream latency\s*:\s*(\d*.\d*)\s*microseconds',
    'units' => 'microsec',
   },
   {'name' => 'Pipe Bandwidth',
    'regex' => 'Pipe bandwidth\s*:\s*(\d*.\d*)\s*MB/sec',
    'units' => 'MB\s',
   },
   {'name' => 'AF_Unix Socket Stream Bandwidth',
