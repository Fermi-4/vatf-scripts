...
static int check_timer(int clock_id)
{
	struct timespec ts;
	int rtn;
	rtn = clock_getres(clock_id, &ts);
	//rtn = clock_getres(CLOCK_REALTIME_HR, &ts);
	//rtn = syscall(__NR_clock_getres, 4, &ts);
	printf("rtn value from clock_getres: %d\n", errno);
	if (rtn)
		return 1;
	/*
	if (clock_getres(CLOCK_MONOTONIC_HR, &ts))
		return 1;
	*/

	//printf("yan >>> the res sec is: %ld\n", ts.tv_sec);
	printf("yan >>> the resolution is: %ldns\n", ts.tv_nsec);
