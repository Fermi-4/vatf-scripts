...
#define DISPLAY_MODE_480P		"480P-60"
#define DISPLAY_MODE_576P		"576P-50"
...
static int in_fmt, out_vid, in_vid;
static struct fb_var_screeninfo vid1_varInfo, vid0_varInfo, osd0_varInfo, osd1_varInfo, prev_vid1_var, prev_vid0_var, prev_osd0_var;
static struct fb_fix_screeninfo vid0_fixInfo, vid1_fixInfo, osd0_fixInfo,osd1_fixInfo;
static int vid0_size, vid1_size, osd0_size, osd1_size;
static int stress_test = 0;
...
static int initialize_capture(v4l2_std_id *);
static int start_loop(void);
static int init_capture_device(void);
static int set_data_format(v4l2_std_id *);
...
static int init_capture_device(void)
{
	...
	input_fmt.type = V4L2_INPUT_TYPE_CAMERA;
	input_fmt.index = 0;
  	while (-EINVAL != ioctl(fdCapture,VIDIOC_ENUMINPUT, &input_fmt)) { 
		printf("input.name = %s\n", input_fmt.name);
		...
		{
		if (!strcmp(input_fmt.name, "COMPONENT"))
			break;
		}
		input_fmt.index++;
  	}

  	if ( ioctl (fdCapture, VIDIOC_S_INPUT, &input_fmt.index) == -1)
	{
		printf (" Init : VIDIOC_S_INPUT \n");
		goto errorexit;
  	}	
  	
	if ( ioctl (fdCapture, VIDIOC_G_INPUT, &vid_type) == -1)
	{
		perror (" Init : VIDIOC_G_INPUT \n");
		goto errorexit;
  	}
  	input_fmt.index = vid_type;

  
  	if ( ioctl (fdCapture, VIDIOC_ENUMINPUT, &input_fmt) == -1)
	{
...
static int set_data_format(v4l2_std_id *cur_std)
{
	...
	*cur_std = prev_std = VPFE_STD_AUTO;
	...
	if(in_fmt == 0 || in_fmt == 2)
	fmt.fmt.pix.height = HEIGHT;
	else if(in_fmt == 1 || in_fmt ==3)
	...
	if(in_fmt == 0 || in_fmt == 1 || in_fmt == 5)
	fmt.fmt.pix.field = V4L2_FIELD_INTERLACED;
	else if(in_fmt == 2 || in_fmt == 3 || in_fmt == 4)
...
#if 1
	/* Set window position */
	pos.xpos = 0;//test_data.vid1_xpos;
	pos.ypos = 0;//test_data.vid1_ypos;
...
static int vpbe_UE_1(void)
{
	...
	if (out_vid == 1) {
		...
		if (in_fmt == 0 || in_fmt == 1)
		{
		test_data.vid1_vmode = FB_VMODE_INTERLACED;
		}
		else if (in_fmt == 3 || in_fmt == 2)
		{
	...
	if(out_vid == 0)
	{
		if (in_fmt == 0 || in_fmt == 2)
		{
			test_data.vid0_width = 720;
			test_data.vid0_height = 480;
		}
		else if (in_fmt == 1 || in_fmt == 3 )
		{
			test_data.vid0_width = 720;
			test_data.vid0_height = 576;
		}
		else if (in_fmt == 4)
		{
			test_data.vid0_width = 1280;
			test_data.vid0_height = 720;
		}
		else if (in_fmt == 5)
		{
			test_data.vid0_width = 1920;
			test_data.vid0_height = 1080;
		}

		test_data.vid0_bpp = 16;
		//test_data.vid0_vmode = FB_VMODE_INTERLACED;
		if (in_fmt == 0 || in_fmt == 1 || in_fmt == 5)
		{
		test_data.vid1_vmode = FB_VMODE_INTERLACED;
		}
		else if (in_fmt == 3 || in_fmt == 2 || in_fmt == 4)
		{
		test_data.vid1_vmode = FB_VMODE_NONINTERLACED;
		}
		test_data.vid0_xpos = 0;
		test_data.vid0_ypos = 0;
		test_data.vid0_hzoom = 0;
		test_data.vid0_vzoom = 0;
	...
	if (in_fmt == 0)
	{
		change_sysfs_attrib(ATTRIB_OUTPUT, "COMPOSITE");
	...
	{
		change_sysfs_attrib(ATTRIB_OUTPUT, "COMPONENT");	
		change_sysfs_attrib(ATTRIB_MODE, DISPLAY_MODE_480P);
	...
	{
		change_sysfs_attrib(ATTRIB_OUTPUT, "COMPONENT");
		change_sysfs_attrib(ATTRIB_MODE, DISPLAY_MODE_720P);
	}
	else if (in_fmt == 5)
	{
		change_sysfs_attrib(ATTRIB_OUTPUT, "COMPONENT");
		change_sysfs_attrib(ATTRIB_MODE, DISPLAY_MODE_1080I);
	}
