...
const char ClassName[] [20] = {"Reserved", "Audio", "Communications", "Human Interface", "Monitor", 
							   "Physical Interface", "Power", "Printer", "Storage", "Hub", "Vendor Specific", 
							   "*ILLEGAL VALUE*"};
const char ConnectionStatus[] [30] = { "No device connected", "Device connected", "Device FAILED enumeration",
...
USHORT DisplayStringDescriptor (HANDLE HubHandle, ULONG PortIndex, USHORT LanguageID, UCHAR Index) {
	...
	if (LanguageID == 0) { // Get the language ID
		...
		Success = DeviceIoControl(HubHandle, IOCTL_USB_GET_DESCRIPTOR_FROM_NODE_CONNECTION, &Packet, sizeof(Packet), &Packet, sizeof(Packet), &BytesReturned, NULL);
...
USHORT DisplayDeviceDescriptor (HANDLE HubHandle, ULONG PortIndex, USHORT LanguageID, PUCHAR BufferPtr) {
	...
	printf("\n bMaxPacketSize0 %2.2x", *++BufferPtr);
	LowByte = *++BufferPtr;
	//printf("\n wVendorID %4.4x", LowByte + (*++BufferPtr << 8));
	printf("\n idVendor %4.4x", LowByte + (*++BufferPtr << 8));
	LowByte = *++BufferPtr;
	//printf("\n wProductID %4.4x", LowByte + (*++BufferPtr << 8));
	printf("\n idProduct %4.4x", LowByte + (*++BufferPtr << 8));
	LowByte = *++BufferPtr;
	//printf("\n wDeviceID %4.4x", LowByte + (*++BufferPtr << 8));
	printf("\n bcdDevice %4.4x", LowByte + (*++BufferPtr << 8));
	...
	while (Length != 0) {
		...
		switch (Type) {
			...
			case 4:
				...
				printf("\n bInterfaceSubClass %2.2x", *++BufferPtr);
				printf("\n bInterfaceProtocol %2.2x", *++BufferPtr);
			...
			case 5:
				...
				printf("\n bmAttributes %2.2x", *++BufferPtr);
				LowByte = *++BufferPtr;
				printf("\n wMaxPacketSize %4.4x", LowByte + ((*++BufferPtr & 0x07)<< 8));
...
void GetPortData(HANDLE HubHandle, UCHAR PortCount, int HubDepth) {
	...
	for (PortIndex = 1; PortIndex < (ULONG)PortCount + 1; PortIndex++) {
		...
		Success = DeviceIoControl(HubHandle, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION, &ConnectionInformation, sizeof(ConnectionInformation), &ConnectionInformation, sizeof(ConnectionInformation), &BytesReturned, NULL);
		...
		if (PortStatus != DeviceConnected) printf("%s\n", ConnectionStatus[PortStatus]);
			...
			if (!ConnectionInformation.DeviceIsHub) {
				...
				Success = DeviceIoControl(HubHandle, IOCTL_USB_GET_NODE_CONNECTION_NAME, &ConnectedHub, sizeof(ConnectedHub), &ConnectedHub, sizeof(ConnectedHub), &BytesReturned, NULL);
				...
				Success = DeviceIoControl(ConnectedHubHandle, IOCTL_USB_GET_NODE_INFORMATION, &NodeInformation, sizeof(NodeInformation), &NodeInformation, sizeof(NodeInformation), &BytesReturned, NULL);
...
DWORD EnumerateHostController(HANDLE HostControllerHandle) {
	...
	Success = DeviceIoControl(RootHubHandle, IOCTL_USB_GET_NODE_INFORMATION, &NodeInformation, sizeof(NodeInformation), &NodeInformation, sizeof(NodeInformation), &BytesReturned, NULL);
	if (!Success) return GetLastError();
