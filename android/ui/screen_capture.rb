...
def run
  ...
  if qual != '' && File.exists?(local_sc_file)
    @equipment['server1'].send_cmd("tar -Jcvf #{local_sc_file}.tar.xz #{local_sc_file}")
    failed_sc_info = upload_file("#{local_sc_file}.tar.xz")
    @results_html_file.add_paragraph("failed_screencap.argb.tar.xz",nil,nil,failed_sc_info[1]) if failed_sc_info
  end
  pre_sysrq = send_adb_cmd("shell ls /sdcard/Pictures/Screenshots").split(/\s+/)
  send_events_for("__sysrq__")
  sleep 5
  send_adb_cmd("shell sync")
  ...
  if (post_syrq - pre_sysrq).empty?
    ...
    if sysrq_qual != '' && File.exists?(local_sysrq_file)
      @equipment['server1'].send_cmd("tar -Jcvf #{local_sysrq_file}.tar.xz #{local_sysrq_file}")
      failed_sc_info = upload_file("#{local_sysrq_file}.tar.xz")
      @results_html_file.add_paragraph("failed_sysrqcap.argb.tar.xz",nil,nil,failed_sc_info[1]) if failed_sc_info
    end
  end
  if qual+sysrq_qual == ''
...
def check_file(ref_argb, test_file)
  ...
  @equipment['server1'].send_cmd("avconv -i #{test_file} -pix_fmt argb -f rawvideo #{test_argb}", @equipment['server1'].prompt, 20)
