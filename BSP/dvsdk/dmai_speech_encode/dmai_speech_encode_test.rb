...
def setup_connect_equipment
 ...
 def run_generate_script
	...
	#+' '+'--compandinglaw '+@test_params.params_chan.companding[0]+' -n'+' '+@test_params.params_chan.num_of_frames[0]
	#puts "test_command is #{test_command}\n"
  ...
  def run_transfer_script()
	...
	if (@test_params.params_chan.input_file[0].to_s.split('.')[1].casecmp("pcm") == 0)
	  subfolder = "/Multimedia/Speech/PCM"
...
def clean_delete_log_files
 ...
 # @equipment['dut1'].send_cmd("del \/Q \*\.asf",@equipment['dut1'].prompt) 
  ...
  #dest_dir = dest_dir+'\video_encode'
   if (!File.exist?(dest_dir))
    puts "Saw that speech_encode in clean_delete_log_files folder does not exist and calling makedirs now\n"
