...
include WinceTestScript
 ...
 # Execute shell script in DUT(s) and save results.
  def run
    ...
    # this sleep is needed to syncronize multimeter and application to run	
   ...
   # Collect voltage output from the multimeter channels,  returns readings and derivative.
  def run_get_multimeter_output(expect_string=nil)
    ...
    while counter < @test_params.params_chan.loop_count[0].to_i         
		@equipment['multimeter1'].send_cmd("READ?", ".*", @test_params.params_chan.timeout[0].to_i, false)
  ...
  # This function calculates  average,min,max,  volttage for all channels. All value are returned in hash table. 
  def sort_raw_data(raw_volt_reading)
	...
	volt_reading_array.each_index{|array_index|
	 ...
	 case mod
	   ...
	   chan_1_volt_readings << volt_reading_array[array_index].gsub(/\+/,'').to_f + 0.00029
	   when  1
	   chan_2_volt_reading += volt_reading_array[array_index].gsub(/\+/,'').to_f + 0.00029
	   chan_2_volt_readings << volt_reading_array[array_index].gsub(/\+/,'').to_f + 0.00029
	   when  2
 ...
 # this function calculates max,min,ava, and individuall power consumption for each domain(channel). 
...
def run_collect_performance_data(power_consumption,voltage_reading)
    ...
    @results_html_file.add_row_to_table(res_table,[power.to_s,power_consumption['all_vvd1'][count].to_s,power_consumption['all_vvd2'][count].to_s,voltage_reading['all']['chan_4'][count].to_s,voltage_reading['all']['chan_5'][count].to_s])
     count += 1	 
	}	
  ensure
  ...
  # The function verifies power consumption. If the power consumption is higher than expected the test fails 
 def verify_power_consumption(power_consumption)
    if @test_params.params_chan.expected_power.to_s.strip > power_consumption['ave_vvd1_plus_vdd2'].to_s.strip then 
      result = 1 
    else 
      result = 0 
    end
 ...
 def run_determine_test_outcome(power_consumption,params_chan, action)
    case params_chan.test_type[0].strip
	when  /power/
    if verify_power_consumption(power_consumption) then 
      puts "-----------test passed---------"
   ...
   when /policy/
	 ...
	 run_get_script_output
    if verify_ips_state(params_chan.ips_to_query,action)  == 1 
      puts "-----------test passed---------"
      result, comment = [FrameworkConstants::Result[:pass], "This test pass."]
    else 
      puts "-----------test failed---------"
