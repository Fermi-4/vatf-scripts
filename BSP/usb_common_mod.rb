...
module UsbCommonModule
	...
	def ck_if_fn_storage_connected
		puts "\n usb_common_module::ck_if_fn_storage_connected "+ __LINE__.to_s
		@rndis_pid = 0
		tmp_run = ''
		@equipment['server1'].send_cmd("C:\\Temp\\devcon listclass usb", />/)
		
		if /VID\_(045e)\&PID\_(FFFF)/i.match(@equipment['server1'].response)
			puts "\n---------- fnStorage is currently available ------ "+ __LINE__.to_s
			@rndis_pid = 99
		else
			puts "\n---------- fnStorage is not available ------ "+ __LINE__.to_s
			@rndis_pid = 0
		end
	end
 ...
 # Collect output from standard output, standard error and serial port in test.log
    ...
    puts "\n Entering usb_common_module::run_get_script_output "+ __LINE__.to_s
    ...
    Dir.foreach(@test_params.params_equip.lcl_dsktp_tmp_dir[0]) do |f|
		...
		#puts "\n Leaving usb_common_module::clean_delete_local_temp_files "+ __LINE__.to_s
	end
  ...
  def run_generate_script
	...
	def init_variables
		...
		@lcl_file_name = @test_params.params_equip.lcl_testfile_name[0] 
		@src_dir = @test_params.params_equip.lcl_dsktp_tmp_dir[0]
		@wince_dst_dir = @test_params.params_chan.instance_variable_defined?(:@test_dir) ? @test_params.params_chan.test_dir[0] : 'C:\Temp'
    ...
    puts "\n usb_common_module::check_file_integrity "+ __LINE__.to_s
		...
		if Digest::SHA2.file(path_to_file_1).hexdigest == Digest::SHA2.file(path_to_file_2).hexdigest
			...
			#puts "\n -------------------- #{@session_success_count} ------ #{@data[@media_type01][9]} ------------------- "+ __LINE__.to_s
		else
			...
			#puts "\n -------------------- #{@session_fail_count} ------ #{@data[@media_type01][10]} ------------------- "+ __LINE__.to_s
		end
    ...
    out_file.puts("dd if=/dev/random of=#{@lcl_dsktop_tmp_dir}" + "\\" + "#{@test_params.params_equip.lcl_testfile_name[0]} bs=1M count=#{@test_params.params_equip.ftp_file_size[0]}")
		...
		system("call #{@tst_file}") ; sleep 1
	end
	
	
	############################################################## check_serial_port #############################################################################
	# In WCE7, the test results created when running TUX, have a "null" character inserted between lines.  This modified routine will remove those null characters.
  def check_serial_port
	...
	def set_dut_datetime
		puts "\n usb_common_module::set_dut_datetime "+ __LINE__.to_s
	...
	def calculate_desired_script_run_time
		puts "\n usb_common_module::calculate_desired_script_run_time "+ __LINE__.to_s
		@run_duration = 0
		
		if @test_params.params_chan.instance_variable_defined?(:@test_duration)
			@run_duration = @test_params.params_equip.instance_variable_defined?(:@test_duration) ? @test_params.params_control.test_duration[0] : '1m'
		end
		...
		case
		when @units == "s"
			@run_duration = @specified_test_duration.to_f * 0
		when @units == "m"
			@run_duration = @specified_test_duration.to_f * 60
		when @units == "h"
			@run_duration = @specified_test_duration.to_f * 60 * 60
		when @units == "d"
			@run_duration = @specified_test_duration.to_f * 60 * 60 * 24
		end
	end
	...
	def calc_current_script_run_time
		...
		@minutes = (@curr_secs / 60)
		@hours = (@curr_secs / (60 * 60))
		@days = (@curr_secs / (60 * 60 * 24))
		puts "\n ------------ Seconds: #{@curr_secs} ----- Days: #{@days} ----- Hours: #{@hours} ----- Mins: #{@minutes} ----- Secs: #{@seconds} ------------ "+ __LINE__.to_s 
    
		@seconds1 = @curr_secs % 60
		@minutes1 = ((@curr_secs / 60 ) % 60)
		@hours1 = (@curr_secs / (60 * 60) % 60)
		@days1 = (@curr_secs / (60 * 60 * 24))
		puts "\n ------------ Seconds: #{@curr_secs} ----- Days: #{@days1} ----- Hours: #{@hours1} ----- Mins: #{@minutes1} ----- Secs: #{@seconds1} ------------ "+ __LINE__.to_s 
	...
	def calc_test_start_time
		puts "\n usb_common_module::calc_test_start_time "+ __LINE__.to_s
		@start_time_seconds = Time.now.strftime("%s")
	end

	
	############################################################## calc_test_end_time ##################################################################
	def calc_test_end_time
		puts "\n usb_common_module::calc_test_end_time "+ __LINE__.to_s
		@end_time_seconds = Time.now.strftime("%s")
	end
  ...
  def xfr_ftp_files()
	...
	def create_evm_info_table
		...
		@test_time = "#{@days} Days  #{@hours} Hours  #{@minutes} Mins. #{@seconds} Secs."
