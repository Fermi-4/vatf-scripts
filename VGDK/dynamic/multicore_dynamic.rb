...
def run
    ...
    (codec_hash).each_pair{|codec,res_arr| 
        ...
        if (res.codec_type == "enc")
          default_params.each_pair do |var,value|
            if(/#{codec}v_enc/).match(var) 
              ...
              dut.send_cmd("dimt set template #{encoder_template} video video_ovly_cfg #{var.gsub("#{codec}v_enc_", "")} #{value}",/OK/,10) 
              else
              dut.send_cmd("dimt set template #{encoder_template} video video_mode #{var.gsub("#{codec}v_enc_", "")} #{value}",/OK/,10) 
              end
            end
          end
          ...
          default_params.each_pair do |var,value|
            if(/#{codec}v_dec/).match(var)
              dut.send_cmd("dimt set template #{decoder_template} video video_mode #{var.gsub("#{codec}v_dec_", "")} #{value}",/OK/,10) 
            end
          end
        end
      #CONF-CONNECT TEMPLATE
        ...
        decoder_template = codec_template_hash[codec]
        encoder_template = decoder_template + 2
        encoder_dyn_template = encoder_template + 7 # template + 5 (worst case 1080p) + 2 (to make it odd)
        case res.codec_type
        when "dec"
  ...
  #  dut.wait_for(/(?:ACK\sDONE)/,60)
    ...
    clip_iter.times { |c_iter|
      ...
      eth_dev = @platform_info.get_eth_dev
      send_threads << Thread.new(iteration_id,eth_dev) { system("#{VIDEO_TOOLS_DIR}/sendPackets.exe #{INPUT_DIR}\\config\\autogenerated\\auto_generated_ConfigFile_4sendPkts_Iter#{iteration_id}.cfg #{eth_dev} 1 s")    
      puts "#{VIDEO_TOOLS_DIR}/sendPackets.exe #{INPUT_DIR}\\config\\autogenerated\\auto_generated_ConfigFile_4sendPkts_Iter#{iteration_id}.cfg #{eth_dev} 1 s"
      sleep(1)
      }
      send_threads << Thread.new() {
        ...
        max_iters.times { |i| 
          ...
          (codec_hash).each_pair{|codec,res_arr| 
            res_arr.each { |res| 
              if res.codec_type == "enc"
                encoder_dyn_template = codec_template_hash[codec] + 2 + 7
...
def set_codec_cfg(dut,codec,res,type,template,var_type,dyn_iter,default_params = nil)
  ...
  if(res != nil ) 
    ...
    when "enc_dyn"
      ...
      if (res == "1080p" && template%2 == 0) # Not a dynamic template; dynamic templates are odd numbered
      ...
      elsif (res == "720p" && template%2 == 0) # Not a dynamic template
...
def channel_reset(dut,tcid)
    ...
    dut.send_cmd("dimt close #{tcid} alloc",/OK/,10)
