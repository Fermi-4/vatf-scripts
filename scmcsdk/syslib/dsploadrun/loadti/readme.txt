...
- Known issue of an internal status message sent to console (CQ #22136)
   
- Known issue where passing in an absolute path + file name for the xml log 
  (via -x option) will cause an error if the file name does not exist. There
  is no issue with relative paths (CQ #21557).
    
- Linux: Known issue where the cycle count is incorrect for some simulators
  (C55x+) (CQ #24284). 
  
- Linux: Known issue where the C55x+ and C64x+ simulators are slowed down by
  the profile clock (CQ #24277). Workaround is to use option '-n' to disable
