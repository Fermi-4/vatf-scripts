# -*- coding: ISO-8859-1 -*-
    ...
    def run
      ...
      #wait_for_string = Regexp.new("Network\sAdded:\s+If-1:((?:\d{1,3}\.){3}\d{1,3})")
      wait_for_string = /Network\sAdded:\s+If-1:((?:\d{1,3}\.){3}\d{1,3})/
     ...
     # connect_to_equipment('dut1')
      ...
      if success_times == boot_times
        ...
        comment = "Test pass. HUA test completed successfully #{boot_times} out of #{boot_times} times "
      else
      test_done_result = FrameworkConstants::Result[:fail]
          comment = "Test fail. HUA test passed #{success_times} out of #{boot_times} times. Boot log - #{boot_arr.to_s} "    
      end
      if (boot_failures > 0) 
    ...
    def parse_response(response,wait_for_string)
      result = response.match(/Network\sAdded:\s+If-1:((?:\d{1,3}\.){3}\d{1,3})/).captures[0].to_s
      puts " +++++++++ Result: #{result} ++++++++++++++"
      if (result)
        return true
      elsif (result == nil)
