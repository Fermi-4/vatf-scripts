...
def run
  ...
  @test_params.params_chan.video_source.each do |vid_source|  
      ...
      File.delete(local_ref_files.last.gsub(/\.mpeg4/, "_test.yuv")) if File.exist?(local_ref_files.last.gsub(/\.mpeg4/, "_test.yuv"))
      ...
      @equipment["dut"].video_decoding({"Source" => local_ref_files.last, "Target" => local_ref_files.last.gsub(/\.mpeg4/, "_test.yuv")})	  
  end
  ...
  result_type = FrameworkConstants::Result[:pass]
    local_ref_files.each do |ref_file|
	  ...
	  num_frames = /_(\d+)frames/.match(ref_file).captures[0].to_i
	  @equipment["video_tester"].decode_file("mpeg4",ref_file, ref_file.gsub(/\.mpeg4/, "_qmaster_dec_ref.yuv"))
#	  ref_file_pixel_format, raw_data_format = get_pixel_and_data_format
	  yuv_to_bmp_params = {     'inFile'            => ref_file.gsub(/\.mpeg4/, "_test.yuv"),
...
#								  'inComponentOrder'  => raw_data_format,
								'inComponentOrder'  => "yuv",
								  ...
								  'outFile'           => ref_file.gsub(/\.mpeg4/, "_test.bmp"),  
	  ...
	  ref_bmp_file.convert(yuv_to_bmp_params.merge({
													'inFile' => ref_file.gsub(/\.mpeg4/, "_qmaster_dec_ref.yuv"),
													'outFile' => ref_file.gsub(/\.mpeg4/, "_qmaster_dec_ref.bmp")}))
	  ref_avi_file = BmpToAviConverter.new
	  bmp_to_avi_params = {     'start'             => ref_file.gsub(/\.mpeg4/, "_test_0000.bmp"), 
								  'end'               => ref_file.gsub(/\.mpeg4/,'_test_'+'0'*(4-(num_frames-1).to_s.length)+(num_frames-1).to_s+'.bmp'), 
								  'frameRate'         => frame_rate.to_s,
								  'output'            => ref_file.gsub(/\.mpeg4/,'_test.avi')}
	  ref_avi_file.convert(bmp_to_avi_params)
	  ref_avi_file.convert(bmp_to_avi_params.merge({
								'start' => ref_file.gsub(/\.mpeg4/,"_qmaster_dec_ref_0000.bmp"),
								'end' =>ref_file.gsub(/\.mpeg4/,'_qmaster_dec_ref_'+'0'*(4-(num_frames-1).to_s.length)+(num_frames-1).to_s+'.bmp'),
								'output' => ref_file.gsub(/\.mpeg4/,'_qmaster_dec_ref.avi')}))
      #======================== Call Q-master ====================================================================
      @equipment["video_tester"].file_to_file_test(ref_file.gsub(/\.mpeg4/,'_qmaster_dec_ref.avi'),
                                                   ref_file.gsub(/\.mpeg4/,'_test.avi'))
        @results_html_file.add_paragraph("")										   
        res_table = @results_html_file.add_table([["#{File.basename(ref_file).gsub(".mpeg4","_test.yuv")} Mean Scores",{:bgcolor => "green", :colspan => "2"},{:color => "red"}]],{:border => "1",:width=>"20%"})
      ...
      if @equipment["video_tester"].get_mos_score < 2
          ...
          result_comment += "#{File.basename(ref_file).gsub(".mpeg4","_test.yuv")} MOS= #{@equipment["video_tester"].get_mos_score}"
          File.cp(ref_file.gsub(".mpeg4","_test.yuv"),ref_file.gsub(".mpeg4","_test_fail_"+Time.now.to_s.gsub(/[\s:-]/,"_")+".yuv"))
          @results_html_file.add_paragraph(ref_file.gsub(".mpeg4","_test_fail_"+Time.now.to_s.gsub(/[\s:-]/,"_")+".yuv"),nil,nil,"//"+local_ref.gsub(".mpeg4","_test_fail_"+Time.now.to_s.gsub(/[\s\:-]/,"_")+".yuv").gsub("\\","/"))
      end
    end
