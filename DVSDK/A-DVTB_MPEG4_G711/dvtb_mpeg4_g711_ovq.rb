...
def setup
  ...
  when /vpfe\+encoder\+decoder\+vpbe/ : 'encdec'
  ...
  if @test_params.params_chan.test_type[0].include?("apfe")
    ...
    @equipment['dut1'].set_param({"Class" => "sphenc", "Param" => "numframes", "Value" => get_audio_num_frames(@test_params.params_control.audio_media_time[0]).to_s})
    ...
    @equipment['dut1'].set_param({"Class" => "audio" , "Param" => "seconds", "Value" => ((get_audio_num_frames(@test_params.params_control.audio_media_time[0])/@test_params.params_chan.audio_sampling_rate[0].to_f/get_audio_frame_size).to_i).to_s})
  end
...
def run
  ...
  @connection_handler.make_video_connection({@equipment["#{@test_params.params_chan.video_iface_type[0]}_converter"] => 0}, {@equipment["dut1"] => video_channel_number, @equipment["tv0"] => 0},@test_params.params_chan.video_iface_type[0]) 
  @connection_handler.make_audio_connection({@equipment["audio_player"] => 0},{@equipment["dut1"] => audio_channel_number, @equipment["tv0"] => 0}, 'mini35mm')   
  @connection_handler.make_audio_connection({@equipment["dut1"] => audio_channel_number},{@equipment["audio_player"] => 0, @equipment['tv1'] => 0}, 'mini35mm')
  @connection_handler.make_video_connection({@equipment["dut1"] => 0},{@equipment['tv1'] => 0}, @test_params.params_chan.video_iface_type[0])
  ...
  @test_params.params_chan.video_source.each do |vid_source|
    ...
    if @test_params.params_chan.test_type[0].include?("vpfe") && @test_params.params_chan.test_type[0].include?("vpbe")
      ...
      @equipment['dut1'].set_param({"Class" => "videnc", "Param" => "numframes", "Value" => get_video_num_frames(vid_source)})
      @equipment['dut1'].set_param({"Class" => "vpfe", "Param" => "numframes", "Value" => get_video_num_frames(vid_source)})
      puts "Encoding+Decoding #{vid_source} ....."
        0.upto(video_channel_number) do |ch_num|
          ...
          if ch_num == video_channel_number
            @equipment['dut1'].video_encoding_decoding
          else
            @equipment['dut1'].video_encoding({"Target" => LOCAL_FILES_FOLDER+vid_source.gsub(".avi","_test_ch"+ch_num.to_s+".mpeg4"), "threadId" => 'mpeg4enc'})
          end
        end
      ...
      video_tester_result = @equipment['video_tester'].video_out_to_video_in_test(vid_enc_dec_params){
        @connection_handler.make_video_connection({@equipment["dut1"] => 0},{@equipment["#{@test_params.params_chan.video_iface_type[0]}_converter"] =>0},@test_params.params_chan.video_iface_type[0])
        sleep 1
      ...
      @connection_handler.make_video_connection({@equipment["#{@test_params.params_chan.video_iface_type[0]}_converter"] => 0},{@equipment["#{@test_params.params_chan.video_iface_type[0]}_converter"] =>0},@test_params.params_chan.video_iface_type[0])  
      @equipment['dut1'].set_param({"Class" => "videnc", "Param" => "numframes", "Value" => (get_te_video_num_frames(vid_source)*2).to_s})
      @equipment['dut1'].set_param({"Class" => "vpfe", "Param" => "numframes", "Value" => (get_te_video_num_frames(vid_source)*2).to_s})
      puts "Encoding #{vid_source} ....."
      video_tester_result = @equipment['video_tester'].video_out_to_file_test(vid_enc_params){
        0.upto(video_channel_number) do |ch_num|
          @equipment['dut1'].set_param({"Class" => "vpfe", "Param" => "chanNumber", "Value" => ch_num.to_s})
          @equipment['dut1'].video_encoding({"Target" => local_ref_file.gsub(/\d+frames/,(get_te_video_num_frames(vid_source)*2).to_s+"frames").gsub(".yuv","_test_ch"+ch_num.to_s+".mpeg4"), "threadId" => 'mpeg4enc'})
        end
        audio_ref_file, audio_test_file, audio_input_handle, audio_output_handle = run_audio_process(audio_channel_number) if @test_params.params_control.audio_num_channels[0].to_i > 0
        @equipment['dut1'].wait_for_threads
        bytes_rec, audio_input_handle, audio_output_handle = stop_audio_process(audio_input_handle, audio_output_handle) if @test_params.params_control.audio_num_channels[0].to_i > 0
        ...
        vid_dec_params = {'ref_clip' => raw_video_ref_file, 'test_clip' =>  test_file, 'format' => @test_params.params_chan.video_signal_format[0], 'data_format' => @test_params.params_chan.video_output_chroma_format[0],
                          ...
                          'video_width' =>@test_params.params_chan.video_width[0], 'num_frames' => get_te_video_num_frames(vid_source), 'frame_rate' => @test_params.params_chan.video_frame_rate[0], 'rec_delay' => @test_params.params_control.video_rec_delay[0].to_f              
                          }
        @connection_handler.make_video_connection({@equipment["#{@test_params.params_chan.video_iface_type[0]}_converter"] => 0},{@equipment["#{@test_params.params_chan.video_iface_type[0]}_converter"] =>0},@test_params.params_chan.video_iface_type[0])  
        ...
        video_tester_result = @equipment['video_tester'].file_to_video_in_test(vid_dec_params){
          ...
          @equipment['dut1'].video_decoding({"Source" => LOCAL_FILES_FOLDER+vid_source, "threadId" => 'mpeg4dec'})
          @connection_handler.make_video_connection({@equipment["dut1"] => 0},{@equipment["#{@test_params.params_chan.video_iface_type[0]}_converter"] =>0},@test_params.params_chan.video_iface_type[0])
          audio_ref_file, audio_test_file, audio_input_handle, audio_output_handle = run_audio_process(audio_channel_number) if @test_params.params_control.audio_num_channels[0].to_i > 0  
        }
        @equipment['dut1'].wait_for_threads
...
def run_audio_process(audio_channel_number)
  ...
  if @test_params.params_chan.test_type[0].include?("apfe") && @test_params.params_chan.test_type[0].include?("apbe")
    ...
    0.upto(audio_channel_number) do |ch_num|
      @equipment['dut1'].speech_encoding_decoding  # Multiple Audio Channel are not supported yet
    end
    ...
    0.upto(audio_channel_number) do |ch_num|
      @equipment['dut1'].speech_encoding({"Target" => test_file, "threadId" => 'g711enc'})  # Multiple Audio Channel are not supported yet
    end
    ...
    ref_file, local_ref_file = get_ref_file(NETWORK_REFERENCE_FILES_FOLDER+'Speech/Encoder/', @test_params.params_chan.audio_source[0]+".pcm")
    @equipment['audio_player'].record_wave_audio(audio_input_handle, test_file)
    @equipment['dut1'].speech_decoding({"Source" => companded_local_ref_file, "threadId" => 'g711dec'})
  end
...
def get_qmaster_avi_file(file_name)
  ...
  ref_bmp_file.convert( {'inFile' => file_name,
               ...
               'outFile' => file_name.gsub(/\.yuv/, "_mpeg4_g711_qmaster_dec.bmp"),  
               'outSamplingFormat' => 'BMP',
               'outFieldFormat'    => 'progressive',
               'outPixelFormat'    => "planar" })
  ref_avi_file = BmpToAviConverter.new
  ref_avi_file.convert({'start'             => file_name.gsub(/\.yuv/, '_mpeg4_g711_qmaster_dec_0000.bmp'), 
              'end'               => file_name.gsub('.yuv','_mpeg4_g711_qmaster_dec_'+'0'*(4-(num_frames-1).to_s.length)+(num_frames-1).to_s+'.bmp'), 
              'frameRate'         => @test_params.params_chan.video_frame_rate[0],
              'output'            => file_name.gsub('.yuv','_mpeg4_g711_qmaster_dec.avi')})
  
  file_name.gsub('.yuv','_mpeg4_g711_qmaster_dec.avi')
end
