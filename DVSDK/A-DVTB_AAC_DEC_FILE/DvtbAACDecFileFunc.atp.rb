...
class DvtbAACDecFileFuncTestPlan < TestPlan
  ...
  def get_outputs(params)
     {
	     'testcaseID'     => "dvtb_aac_dec_file_func.#{@current_id}",
	     'description'    => "AAC Codec Decode File Functionality Test ,with sampling rate "+params['audio_sampling_rate']+", bitrate "+params['audio_bit_rate']+",and audio type "+params['audio_type'], 
	     'ext' => false,
		 'iter' => '1',
		 'bft' => false,
		 'basic' => true,
		 'ext' => false,
		 'bestFinal' => false,
		 'script' => 'Common\A-DVTB_AAC_DEC_FILE\dvtb_aac_dec_file_func.rb',
		 'configID' => '..\Config\dvtb_aac_dec_file_func.ini',
		 'reg'                       => true,
		 'auto'                     => true,
		 'paramsChan'     => {
