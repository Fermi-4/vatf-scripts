...
NETWORK_REFERENCE_FILES_FOLDER = '//gtpegasus/SystemTest_refs/VISA/Image/Decoder'      # TODO: Make sure this is the right place
...
def get_ref_file(file_name)
  case file_name.strip.downcase
    when 'from_encoder'
      jpeg_files = []
      Dir.new(LOCAL_FILES_FOLDER).each { |f| jpeg_files << [LOCAL_FILES_FOLDER+f, LOCAL_FILES_FOLDER+f]  if f =~ /#{@test_params.params_chan.picture_width[0]+'x'+@test_params.params_chan.picture_height[0]}.*\.jpg$/i}
      raise "File #{file_name} not found" if jpeg_files.length == 0
      jpeg_files
    when 'from_web'
      jpeg_files = []
      Dir.new(LOCAL_FILES_FOLDER+'JPEG_Files').each { |f| jpeg_files << [LOCAL_FILES_FOLDER+'JPEG_Files/'+f, LOCAL_FILES_FOLDER+'JPEG_Files/'+f]  if f =~ /#{@test_params.params_chan.picture_width[0]+'x'+@test_params.params_chan.picture_height[0]}.*\.jpg$/i}
      raise "File #{file_name} not found" if jpeg_files.length == 0
      jpeg_files
    else
