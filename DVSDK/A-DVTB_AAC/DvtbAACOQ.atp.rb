...
class DvtbAACOQTestPlan < TestPlan
  ...
  def get_outputs(params)
     {
	     'testcaseID'     => "dvtb_aac_oq.#{@current_id}",
	     'description'    => "AAC Codec Objective Quality Test type "+(params['audio_input_driver']+"+"+params['audio_output_driver']).gsub(/\+{0,1}none\+{0,1}/,"")+",with audio type "+params['audio_type'], 
	     'ext' => false,
		 'iter' => '1',
		 'bft' => false,
		 'basic' => true,
		 'ext' => false,
		 'bestFinal' => false,
		 'script' => 'Common\A-DVTB_AAC\dvtb_aac_oq.rb',
		 'configID' => '..\Config\dvtb_aac_oq.ini',
		 'reg'                       => true,
		 'auto'                     => true,
		 'paramsChan'     => {
  ...
  def get_audio_source(params)
      if params['audio_input_driver'] != "none"
        file_sampling_rate = params['audio_sampling_rate'].to_i/1000
