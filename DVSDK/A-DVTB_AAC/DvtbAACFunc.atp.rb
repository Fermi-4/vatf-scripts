...
class DvtbAACFuncTestPlan < TestPlan
...
#	 'IF [audio_type] = "stereo" AND [audio_input_driver] = "none" THEN [audio_sampling_rate] NOT IN {24000};',
#	 'IF [audio_type] = "mono" AND [audio_input_driver] = "none" THEN [audio_sampling_rate] NOT IN {12000,32000,88000};'
	 ]
  end
  ...
  def get_outputs(params)
     ...
     {
	     'testcaseID'     => "dvtb_aac_func.#{@current_id}",
	     'description'    => "AAC Codec Functionality Test type "+(params['audio_input_driver']+"+"+params['audio_output_driver']).gsub(/\+{0,1}none\+{0,1}/,"")+",with audio type "+params['audio_type'], 
	     'ext' => false,
		 'iter' => '1',
		 'bft' => false,
		 'basic' => true,
		 'ext' => false,
		 'bestFinal' => false,
		 'script' => 'Common\A-DVTB_AAC\dvtb_aac_func.rb',
		 'configID' => '..\Config\dvtb_aac_func.ini',
		 'reg'                       => true,
		 'auto'                     => true,
		 'paramsChan'     => {
   ...
   def get_audio_source(params)
      if params['audio_input_driver'] != "none"
        file_sampling_rate = get_sampling_rate(params).to_i/1000
	  	...
	  	@aac_audio_source_hash["\\w+_"+(get_sampling_rate(params).to_i/1000).to_s+"kHz\\w*"+params['audio_type']+"_{0,1}"+(get_bit_rate(params).to_i/1000).to_s+"kbps\\w*"]
	  end
  end
  
  def get_audio_type(params)
	type_array = []|@params['audio_type']
	if params['audio_type'] == 'stereo' && [24000].include?(get_sampling_rate(params).to_i) 
