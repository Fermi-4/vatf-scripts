...
def run
  ...
  @test_params.params_chan.video_source.each do |vid_source|  
      ...
      File.delete(local_ref_files.last.gsub(/\.yuv/, "_test.mpeg4")) if File.exist?(local_ref_files.last.gsub(/\.yuv/, "_test.mpeg4"))
      ...
      ref_bmp_file.convert( {     'inFile'            => local_ref_files.last,
                                  ...
                                  'outFile'           => local_ref_files.last.gsub('.yuv','.bmp'),                              'outSamplingFormat' => 'BMP',
      ...
      @equipment["dut"].video_encoding({"Source" => local_ref_files.last, "Target" => local_ref_files.last.gsub(/\.yuv/, "_test.mpeg4")})  
  end
  ...
  result_type = FrameworkConstants::Result[:pass]
    local_ref_files.each do |local_ref|
      ...
      @equipment["video_tester"].decode_file("mpeg4",local_ref.gsub(".yuv","_test.mpeg4"),local_qm_decoded_file)                           
      ...
      @equipment["video_tester"].file_to_file_test(local_ref.gsub('.yuv','.avi'),
        ...
        res_table = @results_html_file.add_table([["#{File.basename(local_ref).gsub(".yuv","_test.mpeg4")} Mean Scores",{:bgcolor => "green", :colspan => "2"},{:color => "red"}]],{:border => "1",:width=>"20%"})
      ...
      if @equipment["video_tester"].get_mos_score < 3.5
          ...
          result_comment += "#{File.basename(local_ref).gsub(".yuv","_test.mpeg4")} MOS= #{@equipment["video_tester"].get_mos_score}"
          File.cp(local_ref.gsub(".yuv","_test.mpeg4"),local_ref.gsub(".yuv","_test_fail_"+Time.now.to_s.gsub(/[\s:-]/,"_")+".mpeg4"))
          @results_html_file.add_paragraph(local_ref.gsub(".yuv","_test_fail_"+Time.now.to_s.gsub(/[\s:-]/,"_")+".mpeg4"),nil,nil,"//"+local_ref.gsub(".yuv","_test_fail_"+Time.now.to_s.gsub(/[\s\:-]/,"_")+".mpeg4").gsub("\\","/"))
      end
    end
