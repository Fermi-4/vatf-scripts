...
def setup
  ...
  set_codec_param("deBlocking", "video_deblocking")
  set_codec_param("bottom_fld_DDR_Opt", "video_bottom_fld_ddr_ppt")
  set_codec_param("mb_error_reporting", "video_mb_error_reporting")
  set_codec_param("errorConceal", "video_error_conceal")
end
...
def run
  #======================== Processing Media ====================================================
  begin
    file_res_form = ResultForm.new("MPEG2 Decoder Extended Parameters Test Result Form")
    @test_params.params_chan.video_source.each do |vid_source|
      ...
      @equipment['dut1'].mpeg2ext_decoding({"Source" => local_ref_file, "Target" => target_file, "threadId" => 'mpeg2dec'})
  ...
  if file_res_form.test_result == FrameworkConstants::Result[:fail]
    @equipment['dut1'].get_param({"Class" => "mpeg2extdec", "Param" => ""})
  end
...
def set_codec_param(param_name, param_value)
  if param_value.kind_of?(String)
    @equipment['dut1'].set_param({"Class" => "mpeg2extdec", "Param" => param_name, "Value" => @test_params.params_chan.instance_variable_get('@'+param_value)[0]}) if @test_params.params_chan.instance_variable_defined?('@'+param_value)
  else
    @equipment['dut1'].set_param({"Class" => "mpeg2extdec", "Param" => param_name, "Value" => param_value.to_s})
  end
end