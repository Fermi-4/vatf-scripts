...
def setup
  ...
  set_codec_param("codec", "mpeg4dec")
  ...
  set_codec_param("decRotation", "video_rotation")
  ...
  set_codec_param("outloopDeblocking", "video_outloop_deblocking")
  set_codec_param("outloopDeRinging", "video_outloop_deringing")
  set_codec_param("disableHDVICPeveryFrame", "video_disable_hdvicp_every_frame")
end
...
def run
  ...
  begin
    ...
    @test_params.params_chan.video_source.each do |vid_source|
      ...
      # @equipment['dut1'].set_param({"Class" => "vpfe", "Param" => "chanNumber", "0"})
      set_codec_param("numframes", get_number_of_video_frames(vid_source))
      @equipment['dut1'].mpeg4ext_decoding({"Source" => local_ref_file, "Target" => target_file, "threadId" => 'mpeg4dec'})
...
def set_codec_param(param_name, param_value)
  if param_value.kind_of?(String)
    @equipment['dut1'].set_param({"Class" => "mpeg4extdec", "Param" => param_name, "Value" => @test_params.params_chan.instance_variable_get('@'+param_value)[0]}) if @test_params.params_chan.instance_variable_defined?('@'+param_value)
  else
    @equipment['dut1'].set_param({"Class" => "mpeg4extdec", "Param" => param_name, "Value" => param_value.to_s})
  end
end
