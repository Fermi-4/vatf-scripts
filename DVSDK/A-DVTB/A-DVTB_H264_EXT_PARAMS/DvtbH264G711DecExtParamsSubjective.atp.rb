...
class DvtbH264G711DecExtParamsSubjectiveTestPlan < TestPlan
  # BEG_USR_CFG setup
  # General setup:
  def setup()
    @order = 2
  ...
  def get_params()
    ...
    params = {
    'video_codec' => ['h264extdec','h264fhdextdec'],
    ...
    'video_level' => ['1', '1.1', '1.2', '1.3', '1b', '2','2.1','2.2','3','3.1','3.2', '4', '4.1', '4.2', '5', '5.1'],
    'video_profile' => ['baseline', 'main', 'high', 'any'],
    'video_temp_dir_mode' => [0,1],
  }
  ...
  @video_source_hash = get_source_files_hash("\\w*",video_resolutions,"\\w*",@h264_prof_regex,"\\w*",file_bit_rate,"\\w*frames","264")   
  ...
  def get_outputs(params)
    {
  'description'    =>"H264 Decoder Extended parameters test with, video_res=#{get_video_resolution(params)},bit_rate=#{get_video_bit_rate(params)}",
  
                  
    'iter'                       => '1',
    'testcaseID'                 => "dvtb_h264_g711_dec_ext_params_sub.#{@current_id}",
    'bft'                        => false,
    'basic'                      => false,
    'ext'                        => true,
    'reg'                        => false,
    'auto'                       => true,
    'bestFinal'               => false,
    'script'                     => 'vatf-scripts/DVSDK/A-DVTB/A-DVTB_H264_EXT_PARAMS/dvtb_h264_g711_dec_ext_params_subjective.rb',
 ...
 #   'last'            => true,
  ...
  def get_video_source(params)
    ...
    result = ''
    result = @video_source_hash["\\w*"+video_resolution+"\\w*"+@h264_prof_regex+"\\w*"+file_bit_rate2+"\\w*frames"].to_s if file_bit_rate2
    result += ';' if result != ''
    result += @video_source_hash["\\w*"+video_resolution+"\\w*"+@h264_prof_regex+"\\w*"+file_bit_rate+"\\w*frames"].to_s
    result
  end
  ...
  def get_params_chan(params)
      ...
      result['audio_sampling_rate'] = '8000'
