...
class DvtbAACEncFileFuncTestPlan < TestPlan
...
#	 'IF [audio_type] = "mono" THEN [audio_sampling_rate] <= 48000;',
  ...
  def get_outputs(params)
     ...
     {
	     'testcaseID'     => "dvtb_aac_enc_file_func.#{@current_id}",
	     'description'    => "AAC Codec Encode File Functionality Test ,with sampling rate "+get_sampling_rate(params)+", and audio type "+params['audio_type'], 
	     'ext' => false,
		 'iter' => '1',
		 'bft' => false,
		 'basic' => true,
		 'ext' => false,
		 'bestFinal' => false,
		 'script' => 'Common\A-DVTB_AAC_ENC_FILE\dvtb_aac_enc_file_func.rb',
		 'configID' => '..\Config\dvtb_aac_enc_file_func.ini',
		 'reg'                       => true,
		 'auto'                     => true,
		 'paramsChan'     => {
