...
def setup
  ...
  if @test_params.params_chan.test_type[0].include?("vpfe")
    ...
    @equipment['dut1'].set_param({"Class" => "videnc", "Param" => "codec", "Value" => "h264enc"})
  ...
  if @test_params.params_chan.test_type[0].include?("apfe")
    ...
    @equipment['dut1'].set_param({"Class" => "sphenc", "Param" => "numframes", "Value" => get_audio_num_frames(10).to_s})
    @equipment['dut1'].set_param({"Class" => "sphenc", "Param" => "seconds", "Value" => '10'})
    ...
    @equipment['dut1'].set_param({"Class" => "audio" , "Param" => "seconds", "Value" => '10'})
  ...
  if @test_params.params_chan.test_type[0].include?("apbe")
    ...
    @equipment['dut1'].set_param({"Class" => "sphdec", "Param" => "numframes","Value" => get_audio_num_frames(10).to_s})
    ...
    if @test_params.params_chan.test_type[0].include?("vpfe") && @test_params.params_chan.test_type[0].include?("vpbe")
      ...
      0.upto(video_channel_number) do |ch_num|
        ...
        if ch_num == video_channel_number
          ...
          @equipment['dut1'].video_encoding({"Target" => LOCAL_FILES_FOLDER+vid_source.gsub(".avi","_test_ch"+ch_num.to_s+".264"), "threadId" => 'h264enc'})
        end
      end
      ...
      video_tester_result = @equipment['video_tester'].video_out_to_file_test(vid_enc_params){
        0.upto(video_channel_number) do |ch_num|
          @equipment['dut1'].set_param({"Class" => "vpfe", "Param" => "chanNumber", "Value" => ch_num.to_s})
          @equipment['dut1'].video_encoding({"Target" => local_ref_file.gsub(/\d+frames/,(get_te_video_num_frames(vid_source)*2).to_s+"frames").gsub(".yuv","_test_ch"+ch_num.to_s+".264"), "threadId" => 'h264enc'})
        end
        audio_ref_file, audio_test_file, audio_input_handle, audio_output_handle = run_audio_process(audio_channel_number) if @test_params.params_control.audio_num_channels[0].to_i > 0
        @equipment['dut1'].wait_for_threads
        bytes_rec, audio_input_handle, audio_output_handle = stop_audio_process(audio_input_handle, audio_output_handle) if @test_params.params_control.audio_num_channels[0].to_i > 0
      ...
      vid_dec_params = {'ref_clip' => local_ref_file, 'test_clip' =>  test_file, 'format' => @test_params.params_chan.video_signal_format[0],
                        ...
                        'video_height' => @test_params.params_chan.video_height[0], 'video_width' =>@test_params.params_chan.video_width[0], 'frame_rate' => @test_params.params_chan.video_frame_rate[0], 'rec_delay' => @test_params.params_control.video_rec_delay[0].to_f
      }
      @connection_handler.make_video_connection({@equipment["#{@test_params.params_chan.video_iface_type[0]}_converter"] => 0},{@equipment["#{@test_params.params_chan.video_iface_type[0]}_converter"] =>0},@test_params.params_chan.video_iface_type[0])  
      ...
      video_tester_result = @equipment['video_tester'].file_to_video_in_test(vid_dec_params){
        ...
        @equipment['dut1'].video_decoding({"Source" => LOCAL_FILES_FOLDER+vid_source, "threadId" => 'h264dec'})
        @connection_handler.make_video_connection({@equipment["dut1"] => 0},{@equipment["#{@test_params.params_chan.video_iface_type[0]}_converter"] =>0},@test_params.params_chan.video_iface_type[0])
        audio_ref_file, audio_test_file, audio_input_handle, audio_output_handle = run_audio_process(audio_channel_number) if @test_params.params_control.audio_num_channels[0].to_i > 0  
      }
      @equipment['dut1'].wait_for_threads
...
def map_dut_region(region)
  if region.strip.downcase == "625"
    1.to_s
  else
    0.to_s
  end
