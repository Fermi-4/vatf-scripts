#  [@name: VATF one dut example script
#   @requirements: 12345
#   @params: packages=ti-test, message=hello world
#   @description: VATF test script with one dut. Showcasing a simple Linux
#   test flow, that relies in the default_test_module script to boot the
#   dut to the linux prompt, installs package ti-test, echos hello world
#   and checks that hello world is printed in the console.
#   @setup_requires: dut1=["<platform>",linux];server1=["linuxserver"]
#   @pass_criteria: hello world is seen in console
#   @steps: Power cycle
#           Boot to console
#           Install ti-test package
#           echo hello world
#  ]
...
# Board has booted to prompt and ti-test package is installed
...
def run
  ...
  @equipment['dut1'].send_cmd("echo '#{message}'", /[\r\n]+hello world[\r\n]+#{@equipment['dut1'].prompt}/im)
